...
class AsteroidFieldGenerator(object):
    ...
    def generate(self, t_past, t_future, t_step,
        ...
        yRange = cy_bounds[1] - cy_bounds[0]
        ySpeed = self.bounded_randval(0.0, b_max)
        ticks_across = int(math.floor(yRange / ySpeed))
        cy_start = self.bounded_randval(*cy_bounds)

        a_x, a_y, b_x, b_y, c_x, c_y = (0.0, 0.0, 0.0, -ySpeed, 0.0, cy_start)
        ticks = 0
        while ticks <= t_future:
            ...
            for i in range(3):
                a = Asteroid(a_x=a_x,
                             ...
                             b_y=b_y,
                             c_y=c_y + stack_cnt,
                             t_start=int(math.floor(ticks)))
                asteroids.append(a)
                stack_cnt += min_dist

            ticks = (((c_y - cy_bounds[0]) / yRange) * ticks_across) + ticks
            c_y = cy_bounds[1]
